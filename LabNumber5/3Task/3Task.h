...
long long *FillVector(long long **matrix, long long &vecSize, bool &EndFlag) {
    try {
        int newVecSize = 0;
        if(vecSize <= 0)
            throw vecSize;
        ...
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 4);
        std::cerr << "Incorrect array size(no even : " << err << '\n';
        std::cerr << "Try with another input!\n";
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 2);
        EndFlag = true;
...
void showVector(long long* vector, size_t vecSize, bool &EndFlag)
{
...
void SolveTask(){
    while(true)
    {
        ...
        bool EndFlag = false;
        newVector = FillVector(matrix, vecSize, EndFlag);
        if(!EndFlag)
        showVector(newVector, vecSize, EndFlag);
        if(RestartProgram())
            break;
    }
}
